...
def train(
    ...
    if device.type == 'cpu':
        elap, loss = train_cpu(model, device, optimizer, data_type,
                input_size, output_size, batch_size, args)

    elif device.type == 'cuda':
        elap, loss = train_gpu(model, device, optimizer, data_type,
                input_size, output_size, batch_size, args)

    elif device.type == 'xla':
        elap, loss = train_tpu(model, device, optimizer, data_type,
                input_size, output_size, batch_size, args)
...
def run(args, dataset):
    ...
    for i in range(len(dataset)):
        ...
        elap, loss = run_single(args, layer_num, input_size, hidden_size, output_size, batch_size)
        elap /= args.steps

        flops = batch_size * (hidden_size * hidden_size * layer_num
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dtype", default="float", help="data type", choices=["float", "float16", "bfloat16"])
